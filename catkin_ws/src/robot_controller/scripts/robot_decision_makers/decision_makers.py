...
class DecisionMakers:
    def __init__(self):
        ...
        self.semantic_mapping_mark_topic = rospy.get_param("~semantic_mapping_mark_topic", "/semantic_mapping_mark")
        self.semantic_mapping_failed_action = rospy.get_param("~semantic_mapping_failed_action", "left---continue")
        ...
        self._last_mark_pub_time = rospy.Time(0)
        try:
        ...
        self.semantic_mapping_mark_sub = rospy.Subscriber(
            self.semantic_mapping_mark_topic,
            String,
            self.semantic_mapping_mark_callback,
            queue_size=10
        )
    ...
    def semantic_mapping_mark_callback(self, msg: String):
        mark = (msg.data or "").strip().lower()
        if not mark:
            return

        # only react to "failed"
        if mark != "failed":
            self._last_mark = mark
            return

        # debounce: publish at most once per 0.5s
        now = rospy.Time.now()
        if (self._last_mark == "failed") and ((now - self._last_mark_pub_time).to_sec() < 0.5):
            return

        self._last_mark = "failed"
        self._last_mark_pub_time = now

        out = String()
        out.data = self.semantic_mapping_failed_action  # default "left---continue"
        rospy.logwarn("[DecisionMakersNode] semantic_mapping_mark=failed -> publishing: %s", out.data)
        self.next_action_pub.publish(out)
