...
class MultiObjectGraspStoreWithTrackingID:
    ...
    def __init__(self):
        ...
        # Candidate stability gate (helps ghost flicker)
        self.min_instance_hits = rospy.get_param("~min_instance_hits", 3)
        self.min_instance_age = rospy.get_param("~min_instance_age", 0.20)  # seconds
        ...
        self.search_backup_speed = rospy.get_param("~search_backup_speed", 0.10)
        ...
        # Trials / robustness
        self.max_trials_per_instance = rospy.get_param("~max_trials_per_instance", 3)
        self.attempt_pose_bin = rospy.get_param("~attempt_pose_bin", 0.08)  # meters/bin
        self.max_approach_rejects_per_pose = rospy.get_param("~max_approach_rejects_per_pose", 3)
        self.max_backup_entries_no_progress = rospy.get_param("~max_backup_entries_no_progress", 6)
        ...
        # ---------------- Detection DB ----------------
        # key: (label, tid) -> {"x","z","stamp","rawx","rawz","hits","first_seen"}
        self.instances = {}
        ...
        self.lock_pose_key = None  # pose-binned key (should be frozen at selection!)
        ...
        # Attempts/rejects keyed by pose
        self.pose_attempts = {}
        self.pose_rejects = {}

        # Oscillation guard
        self.backup_entries_no_progress = 0
    ...
    def transition(self, s):
        ...
        self.state = s
        self.state_enter_time = rospy.Time.now()
        if s in (self.SEARCH_BACKUP, self.VERIFY_STALE_BACKUP) and prev != s:
            self.backup_entries_no_progress += 1
            rospy.logwarn(f"[OscGuard] backup_entries_no_progress="
                          f"{self.backup_entries_no_progress}/{self.max_backup_entries_no_progress}")
    ...
    # Pose-key (bin)
    def pose_key(self, label, x, z):
        b = max(self.attempt_pose_bin, 1e-6)
        bx = int(round(float(x) / b))
        bz = int(round(float(z) / b))
        return (str(label), bx, bz)

    def attempts_for_key(self, pkey):
        return int(self.pose_attempts.get(pkey, 0))

    def rejects_for_key(self, pkey):
        return int(self.pose_rejects.get(pkey, 0))

    def is_blacklisted_key(self, pkey):
        return (self.attempts_for_key(pkey) >= int(self.max_trials_per_instance)) or \
               (self.rejects_for_key(pkey) >= int(self.max_approach_rejects_per_pose))

    def register_attempt_start_key(self, pkey):
        self.pose_attempts[pkey] = self.attempts_for_key(pkey) + 1
        rospy.logwarn(f"[Attempt] {pkey} -> {self.pose_attempts[pkey]}/{self.max_trials_per_instance}")

    def register_reject_key(self, pkey, reason):
        self.pose_rejects[pkey] = self.rejects_for_key(pkey) + 1
        rospy.logwarn(f"[Reject] {pkey} -> {self.pose_rejects[pkey]}/{self.max_approach_rejects_per_pose} ({reason})")

    def clear_pose_counters(self, pkey):
        self.pose_attempts.pop(pkey, None)
        self.pose_rejects.pop(pkey, None)

    def note_failure_key(self, pkey, reason):
        rospy.logwarn(f"[Fail] {pkey} reason={reason} attempts={self.attempts_for_key(pkey)}/{self.max_trials_per_instance} "
                      f"rejects={self.rejects_for_key(pkey)}/{self.max_approach_rejects_per_pose}")

    # Stable-only visibility (ghost filter)
    def visible_instances_for_label(self, label):
        ...
        for (lb, tid), d in self.instances.items():
            ...
            first_seen = d.get("first_seen", d["stamp"])
            age_seen = (now - first_seen).to_sec()
            if hits < self.min_instance_hits:
                continue
            if age_seen < self.min_instance_age:
                continue
            out.append((tid, d["x"], d["z"]))
        return out

    def any_selectable_visible_matching_targets(self):
        for label, cnt in self.targets.items():
            if cnt <= 0:
                continue
            for (tid, x, z) in self.visible_instances_for_label(label):
                pkey = self.pose_key(label, x, z)
                if not self.is_blacklisted_key(pkey):
                    return True
        return False

    def clear_lock_and_pre(self):
        self.lock_label = None
        self.lock_tid = None
        self.lock_pose_key = None
        self.pre_label = None
        self.pre_label_count = None
        self.pre_label_ids = set()
        self.verify_absent_count = 0
    ...
    def cb_det(self, msg: PointStamped):
        ...
        if k in self.instances:
            hits = int(self.instances[k].get("hits", 0)) + 1
            first_seen = self.instances[k].get("first_seen", now)
        else:
            hits = 1
            first_seen = now
    ...
    def step_select_instance(self):
        ...
        for label in self.target_order:
            ...
            candidates.sort(key=lambda t: t[2])  # nearest first (min z)

            for (tid, x, z) in candidates:
                pkey = self.pose_key(label, x, z)
                if self.is_blacklisted_key(pkey):
                    rospy.logwarn_throttle(1.0, f"[Select] Skip blacklisted {pkey}")
                    continue
                chosen_label, chosen_tid, chosen_x, chosen_z, chosen_pkey = label, tid, x, z, pkey
                break
        ...
        if chosen_label is None:
            rospy.logwarn_throttle(1.0, "[Select] No selectable candidates. SEARCH_BACKUP.")
        ...
        # ### FIX: freeze pose-key from SELECTION coordinates (pre-approach), do NOT recompute later
        self.lock_pose_key = chosen_pkey

        self.stable_count = 0
        rospy.loginfo(f"[Select] Locked: {self.lock_label}:{self.lock_tid} key={self.lock_pose_key} "
                      f"(remaining={self.targets.get(self.lock_label,0)})")
    ...
    def step_approach(self):
        ...
        if self.lock_pose_key is not None and self.is_blacklisted_key(self.lock_pose_key):
            rospy.logwarn(f"[Approach] Pose is blacklisted {self.lock_pose_key}. Reselect.")
            self.clear_lock_and_pre()
            self.transition(self.SELECT_INSTANCE)
            return
        ...
        if k not in self.instances:
            self.publish_stop(force=False)
            if self.lock_pose_key is not None:
                self.register_reject_key(self.lock_pose_key, "missing_during_approach")
            self.clear_lock_and_pre()
            self.transition(self.SELECT_INSTANCE)
            return
        ...
        if not self.is_fresh_instance(self.lock_label, self.lock_tid):
            self.publish_stop(force=False)
            if self.lock_pose_key is not None:
                self.register_reject_key(self.lock_pose_key, "stale_during_approach")
            self.clear_lock_and_pre()
            self.transition(self.SELECT_INSTANCE)
            return
        ...
        if self.stable_count >= self.stable_cycles_required:
            ...
            # ### FIX: DO NOT overwrite lock_pose_key here.
            # All objects become x≈0,z≈target_depth after approach, so overwriting would collapse keys.

            self.last_grasp_pose_key = self.lock_pose_key
    ...
    def step_grasp_cmd(self):
        ...
        if self.lock_pose_key is None:
            # fallback: use the last seen (still better than post-center overwrite)
            self.lock_pose_key = self.pose_key(self.lock_label, self.lock_last_x or 0.0, self.lock_last_z or self.target_depth)
        self.last_grasp_pose_key = self.lock_pose_key

        if self.is_blacklisted_key(self.lock_pose_key):
            rospy.logerr(f"[GraspCmd] Refusing: blacklisted {self.lock_pose_key}")
            self.clear_lock_and_pre()
            self.transition(self.SELECT_INSTANCE)
            return

        self.register_attempt_start_key(self.lock_pose_key)

        self.grasp_pub.publish(Int32(data=1))
        rospy.loginfo(f"[Grasp {self.lock_label}:{self.lock_tid}] Published grasp cmd=1 (one-shot) key={self.lock_pose_key}")
    ...
    def step_wait_grasp(self):
        ...
        if elapsed > self.grasp_wait_timeout:
            rospy.logerr(f"[Grasp] Timeout >{self.grasp_wait_timeout:.1f}s.")
            if self.lock_pose_key is not None:
                self.note_failure_key(self.lock_pose_key, "grasp_timeout")
            self.clear_lock_and_pre()
            self.transition(self.SELECT_INSTANCE)
    ...
    def step_wait_store(self):
        ...
        if elapsed > self.store_wait_timeout:
            rospy.logerr(f"[Store] Timeout >{self.store_wait_timeout:.1f}s.")
            if self.lock_pose_key is not None:
                self.note_failure_key(self.lock_pose_key, "store_timeout")
            self.clear_lock_and_pre()
            self.transition(self.SELECT_INSTANCE)
    ...
    def step_verify_removed(self):
        ...
        removed_condition = count_decreased or ((not pose_occupied) and locked_absent)
        ...
        if self.verify_absent_count >= self.verify_absent_cycles:
            ...
            try:
                ...
                rospy.logerr(f"[VerifyRemoved] Failed writing targets file: {e}")

            rospy.loginfo(f"[Targets] Vision-confirmed stored. Updated: {self.targets}")
            self.backup_entries_no_progress = 0

            if pkey is not None:
                self.clear_pose_counters(pkey)
        ...
        if elapsed > self.verify_timeout:
            rospy.logwarn(f"[VerifyRemoved] Timeout >{self.verify_timeout:.1f}s. Not decrementing.")
            if pkey is not None:
                self.note_failure_key(pkey, "verify_timeout")
            self.clear_lock_and_pre()
            self.transition(self.SELECT_INSTANCE)

    def step_verify_stale_backup(self):
        if self.backup_entries_no_progress > self.max_backup_entries_no_progress:
            rospy.logerr("[OscGuard] Too many backup cycles without progress -> FINISH.")
            self.transition(self.FINISH)
            return
        ...
        rospy.logwarn("[VerifyStaleBackup] still no detections after backup -> reselect.")
        self.clear_lock_and_pre()
        self.transition(self.SELECT_INSTANCE)
    ...
    def step_search_backup(self):
        ...
        if self.any_selectable_visible_matching_targets():
            self.transition(self.SELECT_INSTANCE)
            return
        ...
        if self.any_selectable_visible_matching_targets():
            rospy.loginfo("[SearchBackup] selectable target found after backup. Reselect.")
            self.transition(self.SELECT_INSTANCE)
            return

        if self.finish_if_missing_after_backup:
            rospy.logwarn("[SearchBackup] no selectable targets after backup. Finishing.")
            self.transition(self.FINISH)
        else:
            self.transition(self.SELECT_INSTANCE)
