...
class MultiObjectGraspStoreWithTrackingID:
    ...
    def __init__(self):
        ...
        # ---------------- Detection DB (must exist BEFORE subscribing) ----------------
        self.instances = {}
        ...
        # ---------------- MINIMAL TWEAK ----------------
        # Track whether our last cmd_vel was already (0,0). If yes, don't republish zeros.
        self._last_cmd_was_zero = True

        rospy.loginfo("[MultiObjectTrackingID] Node started (minimal tweaks).")
    ...
    def publish_stop(self, force=False):
        """
        MINIMAL TWEAK:
        - Do NOT spam cmd_vel zeros every cycle.
        - Only publish zero ONCE when we were previously sending non-zero.
        """
    ...
    def publish_cmd(self, vx, vy):
        """
        Publish motion command only when non-zero.
        If zero, call publish_stop() (one-shot).
        """
    ...
    def reset_run_state(self):
        # run-time reset only (keep targets + detections buffer)
        self.publish_stop(force=True)  # stop once on reset/deactivate
        self.lock_label = None
        self.lock_tid = None
        self.lock_last_x = None
        self.lock_last_z = None
    ...
    def cb_activate(self, msg: Int32):
        if msg.data == 1:
            ...
            self.publish_stop(force=True)  # take control immediately, but only once
            self.lock_label = None
            self.lock_tid = None
            self.grasp_success = False
            self.store_success = False
    ...
    def cb_det(self, msg: PointStamped):
        ...
        self.instances[k] = {"x": fx, "z": fz, "rawx": x, "rawz": z, "stamp": rospy.Time.now()}
    ...
    def step_wait_activation(self):
        # MINIMAL TWEAK: do NOT spam stop while waiting/inactive
        self.publish_stop(force=False)
    ...
    def step_approach(self):
        ...
        # MINIMAL TWEAK: don't publish (0,0) every cycle
    ...
    def step_grasp_cmd(self):
        self.publish_stop(force=False)
        self.grasp_pub.publish(Int32(data=1))  # one-shot
    ...
    def step_store_cmd(self):
        self.publish_stop(force=False)
        self.store_pub.publish(Int32(data=1))  # one-shot
    ...
    def step_wait_store(self):
        ...
        if self.store_success:
            rospy.loginfo("[Store] Success=1. Verify removed...")
    ...
    def step_verify_removed(self):
        ...
        still_there = False
        if k in self.instances and self.is_fresh_instance(label, tid):
            x = self.instances[k]["x"]
            z = self.instances[k]["z"]
        ...
        rospy.loginfo_throttle(
            0.25,
            f"[VerifyRemoved {label}:{tid}] still_there={still_there} "
            f"absent={self.verify_absent_count}/{self.verify_absent_cycles} elapsed={elapsed:.2f}s"
        )
        ...
        if self.verify_absent_count >= self.verify_absent_cycles:
            ...
            rospy.loginfo(f"[Targets] Removed confirmed. Updated: {self.targets}")
    ...
    def step_search_backup(self):
        ...
        if elapsed < self.search_backup_time:
            # This is a real motion command -> publish nonzero
