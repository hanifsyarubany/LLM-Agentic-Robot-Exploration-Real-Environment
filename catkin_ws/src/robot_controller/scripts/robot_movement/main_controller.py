...
class MainControllerNode(object):
    def __init__(self):
        ...
        # NEW: Post-grasp backup (distance is approximated via time = distance/speed)
        self.post_grasp_backup_distance = float(rospy.get_param("~post_grasp_backup_distance", 1.0))  # [m]
        self.post_grasp_backup_speed    = float(rospy.get_param("~post_grasp_backup_speed", 0.25))    # [m/s] magnitude
        ...
        # cmd_vel publisher for post-grasp turning / backup
        ...
        #           "OBJECT_GRASPING", "POST_GRASP_TURN", "POST_GRASP_BACKUP",
        #           "PICKUP_STATE"
        # -------------------------------------------------------------
    ...
    # -------------------------------------------------------------
    # NEW: Post-grasp backup helpers
    # -------------------------------------------------------------
    def start_post_grasp_backup(self, now):
        dist = abs(self.post_grasp_backup_distance)
        speed = abs(self.post_grasp_backup_speed) if abs(self.post_grasp_backup_speed) > 1e-6 else 0.25
        duration = dist / speed if speed > 1e-6 else 0.0

        self.backup_start_time = now
        self.backup_duration = duration
        self.backup_lin_x = -speed  # backward
        self.state = "POST_GRASP_BACKUP"

        rospy.loginfo("[MainController] Post-grasp backup: dist=%.2fm, speed=%.2fm/s, duration=%.2fs",
                      dist, speed, duration)

        t = Twist()
        t.linear.x = self.backup_lin_x
        self.cmd_vel_pub.publish(t)

    def stop_post_grasp_backup(self):
        self.cmd_vel_pub.publish(Twist())
        self.backup_start_time = None
        self.backup_duration = None
        self.backup_lin_x = 0.0
    ...
    def control_loop(self, event):
        ...
        if self.state == "POST_GRASP_TURN":
            ...
            rospy.loginfo("[MainController] Post-grasp turn complete. Starting backup motion...")
            self.stop_post_grasp_turn()
            self.start_post_grasp_backup(now)
            return

        # POST_GRASP_BACKUP
        if self.state == "POST_GRASP_BACKUP":
            if self.backup_start_time is None or self.backup_duration is None:
                rospy.logwarn("[MainController] POST_GRASP_BACKUP missing timing; stopping and returning to WALL_AVOID.")
                self.stop_post_grasp_backup()
                self.switch_to_wall_avoid()
                return

            elapsed = (now - self.backup_start_time).to_sec()
            if elapsed < self.backup_duration:
                t = Twist()
                t.linear.x = self.backup_lin_x
                self.cmd_vel_pub.publish(t)
                rospy.logdebug_throttle(1.0, "[MainController] POST_GRASP_BACKUP in progress...")
                return

            rospy.loginfo("[MainController] Post-grasp backup complete. Returning to WALL_AVOID.")
            self.stop_post_grasp_backup()
            self.switch_to_wall_avoid()
            return
