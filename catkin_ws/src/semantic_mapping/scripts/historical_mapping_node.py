...
class HistoricalMappingNode:
    def __init__(self):
        ...
        # NEW: user order-list prompt input
        self.input_order_topic = rospy.get_param("~input_order_topic", "/input_order_list")
        self._input_order_sub = rospy.Subscriber(self.input_order_topic, String,   		self.input_order_cb, queue_size=10)
        rospy.loginfo("  input_order_topic:%s", self.input_order_topic)
    ...
    def input_order_cb(self, msg: String) -> None:
        ts = self._ts()
        text = (msg.data or "").strip()
        if not text:
            return
        # avoid breaking log format if user sends multiline
        text = " ".join(text.splitlines()).strip()
        self._append_line(f"[{ts}] The robot receives a command from the user: {text}")
