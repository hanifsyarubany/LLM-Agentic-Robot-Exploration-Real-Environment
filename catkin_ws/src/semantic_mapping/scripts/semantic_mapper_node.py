...
class SemanticMapper(object):
    def __init__(self):
        ...
        self._tick = rospy.Timer(rospy.Duration(0.1), self._timer_cb)
    ...
    def _timer_cb(self, _evt):
        if not self.session_active or self.session_finished:
            return
        self.session_last_time = rospy.Time.now().to_sec()
    ...
    def _start_new_session(self):
        ...
        self.session_finished = False
        self.session_start_time = rospy.Time.now().to_sec()
        self.session_last_time = self.session_start_time
        self.detections_by_secs = defaultdict(list)
